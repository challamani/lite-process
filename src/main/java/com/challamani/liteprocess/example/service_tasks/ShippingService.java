...
@Service
@Slf4j
public class ShippingService implements LiteTaskExecutor<ServiceTask> {
    @Override
    public void execute(ServiceTask serviceTask) {
        ...
        log.info("<<< ShippingService order {} status {}", orderId, orderStatus);
        if(orderStatus.equalsIgnoreCase("ReadyToShip")){
            log.info("<<< ShippingService is completed for the order {}", orderId);
